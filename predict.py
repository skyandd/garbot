...
prototxt_path = '/models/deploy.prototxt'  # Путь до opencv модели
caffemodel_path = '/models/weights.caffemodel'  # Путь до opencv модели
...
file = os.listdir(path=dir_path)[0]
...
cv2.imwrite(dir_faces_recognition + file, image)
...
for i in range(0, detections.shape[2]):
    ...
    if confidence > 0.4:
        ...
        cv2.imwrite(dir_faces_recognition + file + '_' + 'person' + str(i), frame)
...
counter = 0
title_obj = str()
for filename in os.listdir(path="/content/drive/My Drive/save_fase_detection/"):
    counter += 1
    img = tfimage.load_img("/content/drive/My Drive/save_fase_detection/" + filename, target_size=(224, 224))
